  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-black text-neutral-100' : 'bg-white text-neutral-900'} font-sans flex flex-col lg:flex-row h-screen overflow-hidden transition-theme`}>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar flex flex-col relative">
        <div className="max-w-full mx-auto w-full p-4 lg:p-4 flex flex-col gap-4">
          ...
          <header className={`hidden lg:flex items-center justify-between sticky top-0 z-50 py-4 px-8 border-b transition-all ${theme === 'dark' ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200'} backdrop-blur-xl`}>
             <div className="flex items-center gap-4">
                ...
                {currentView === 'list' && (
                  <div className={`flex p-1 rounded-full ${theme === 'dark' ? 'bg-black' : 'bg-neutral-100'} border ${theme === 'dark' ? 'border-neutral-800' : 'border-neutral-200'}`}>
                    {['movie', 'series'].map(m => (
             ...
             <div className="flex items-center gap-3">
                {currentView === 'list' && <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 opacity-30" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`pl-8 pr-4 py-1.5 rounded-lg text-[10px] outline-none border transition-all ${theme === 'dark' ? 'bg-black border-neutral-800 focus:border-indigo-500' : 'bg-neutral-50 border-neutral-200 focus:border-indigo-400'}`} placeholder="Search IMDB or Files..." /></div>}
          ...
          {currentView === 'upload' ? (
              ...
              <div className={`rounded-[1rem] border p-6 lg:p-10 ${theme === 'dark' ? 'bg-black border-neutral-800 ' : 'bg-white border-neutral-100 '}`}>
                <form onSubmit={handleUploadSubmit} className="space-y-8">
                  <div className="grid grid-cols-2 gap-4">
                    {['movie', 'series'].map(t => (
                      <button key={t} type="button" onClick={() => setUploadForm({...uploadForm, type: t})} className={`p-6 rounded-[0.8rem] border-2 transition-all flex flex-col items-center gap-3 ${uploadForm.type === t ? `${a.main} border-transparent text-white  ${a.shadow}` : `${theme === 'dark' ? 'bg-[#0a0a0a] border-neutral-800' : 'bg-neutral-50 border-neutral-100'}`}`}>
                  ...
                  <button type="button" onClick={handleUploadSubmit} disabled={!uploadForm.imdbId || stagedFiles.length === 0 || isUploading} className={`w-full py-5 rounded-2xl font-black text-white transition-all  ${!uploadForm.imdbId || stagedFiles.length === 0 || isUploading ? 'opacity-20 cursor-not-allowed' : `${a.main} ${a.hover} ${a.shadow} active:scale-95`}`}>{isUploading ? 'Synchronizing Cluster...' : `Commit ${stagedFiles.length} Subtitles`}</button>
                          ...
                          return (
                            <div key={idx} className={`p-6 rounded-[0.8rem] border-2 flex flex-col gap-4 group transition-all hover:scale-[1.02] ${theme === 'dark' ? 'bg-[#0a0a0a] border-neutral-800 hover:border-indigo-500/50' : 'bg-neutral-50 border-neutral-100 hover:border-indigo-400'}`}>
                               <div className="flex gap-4 items-start">
                                  {result.thumbnail ? (
                                    <img src={result.thumbnail} className="w-16 h-20 object-cover rounded-xl " alt="" />
                                  ) : (
                                    <div className="w-16 h-20 bg-black rounded-xl flex items-center justify-center"><Film className="w-6 h-6 opacity-20" /></div>
                                  )}
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                     <div className="space-y-1">
                                        ...
                                        <input 
                                          ...
                                          className={`w-full py-2 px-3 rounded-lg text-[10px] font-mono border ${theme === 'dark' ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200'}`}
                                     ...
                                     <div className="space-y-1">
                                        ...
                                        <div className="relative">
                                          <select 
                                            className={`w-full py-2 px-3 rounded-lg text-[10px] font-black uppercase border appearance-none cursor-pointer ${theme === 'dark' ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200'}`}
                                   ...
                                   <div className="flex gap-2">
                                      <input 
                                        type="number" 
                                        placeholder="S" 
                                        className={`w-14 py-2 px-2 rounded-lg text-[10px] border [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${theme === 'dark' ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200'}`}
            ...
            <div className="w-full animate-in fade-in duration-700">
               <div className={`rounded-[0.8rem] border overflow-hidden ${theme === 'dark' ? 'bg-[#0a0a0f] border-neutral-800 ' : 'bg-black border-neutral-800 '}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4 pb-20 animate-in fade-in duration-700">
               {Object.entries(subtitles.filter(sub => {
                 ...
                 if (!acc[key]) acc[key] = []; acc[key].push(sub); return acc;
                  ...
                  return (
                     <div key={groupKey} className={`flex flex-col h-full border rounded-[0.8rem] overflow-hidden transition-all duration-300 ${theme === 'dark' ? 'bg-black border-neutral-800' : 'bg-white border-neutral-200'}`}>
                        <div className="relative h-32 shrink-0 overflow-hidden">
                           ...
                           <div className="absolute inset-0 p-4 flex gap-3 items-end">
                              {m?.poster_path && <img src={m.poster_path} className="w-12 h-18 object-cover rounded-lg " />}
                              <div className="flex-1 min-w-0">
                                 <div className="flex flex-wrap gap-1 mb-1 items-center">
                                    <span className={`text-[6px] font-black uppercase px-2 py-0.5 rounded-full ${isSeries ? 'bg-purple-600' : 'bg-indigo-600'} text-white `}>{first.type}</span>
                                    {isSeries && first.season != null && <span className="text-[6px] font-black uppercase px-2 py-0.5 rounded-full bg-neutral-700 text-white ">S{String(first.season).padStart(2,'0')}</span>}
